            ...
            <form @submit.prevent="searchYoutube()">
              ...
              <div class="youtube">
                <div v-for="youtube in youtubes" :key="youtube.id.videoId">
                  <!-- <a
                    :href="
                      'https://www.youtube.com/embed/' + youtube.id.videoId
                    "
                    target="_blank"
                  > -->
...
export default {
  ...
  setup() {
    const search = ref("blackpink");
    const youtubes = ref([]);

    const searchYoutube = () => {
      fetch(
        `https://youtube.googleapis.com/youtube/v3/search?part=snippet&maxResults=30&q${search.value}&type=video&key=${env.apikey}`
      )
